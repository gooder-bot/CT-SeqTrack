...
def apply_b1_calibration_contract(model, payload, state_dict):
    ...
    if bool(getattr(model, "require_b1_calibration_artifact", False)):
        ...
        if (
            source_metadata.get("partition") != "calibration"
            or source_metadata.get("dataset")
            != str(getattr(model.config, "dataset", "unknown"))
            or source_metadata.get("split")
            != str(getattr(model.config, "train_split", "train"))
    ...
    if bool(getattr(model, "require_b1_calibration_passed", False)):
        if not isinstance(calibration, dict) or not bool(
            calibration.get("promotion", {}).get("passed")
        ):
...
def load_evaluation_weights(model, checkpoint_path, report_path=None):
    """Load shape-compatible tensors for an offline evaluation/export job.

    This utility is intentionally outside the training entry point: it loads
    the model being inspected by an exporter and cannot initialize or resume a
    training run.
    """
    try:
    ...
    if nonfinite:
        raise RuntimeError(
            "checkpoint contains non-finite tensors: " + ", ".join(nonfinite[:20])
        )
