...
def select_b2_v3_history_mode(tracklet_key, frame_id, candidate_id, seed=42):
    ...
    if candidate_id == 0:
        return "canonical"
    payload = (
        f"b2-v3::{int(seed)}::{str(tracklet_key)}::" f"{int(frame_id)}::{candidate_id}"
    ).encode("utf-8")
    bucket = hashlib.sha256(payload).digest()[0] & 1
    return "correlated_candidate" if bucket == 0 else "recursive_candidate"
...
def correlate_candidate_offsets(offsets, correlation, anchor_offset):
    ...
    for index in range(1, len(offsets)):
        correlated[index] = (
            correlation * correlated[index - 1] + (1.0 - correlation) * offsets[index]
        )
...
def build_ct_history_offsets(
    ...
    if candidate_trajectory_mode != "independent":
        raise ValueError("candidate_trajectory_mode must be independent or shared_se2")
    ...
    if training_mode == "recursive_candidate":
        ...
        age_scale = np.sqrt(np.arange(len(motion_offsets), dtype=np.float32) + 1.0)
        age_scale[0] = 0.0
        motion_offsets = (
            motion_offsets * age_scale[:, None] * recursive_error_scale
        ).astype(np.float32)
        # The newest box remains the actual crop anchor while older history
        # accumulates correlated drift relative to it.
        search_offsets = (candidate_offsets[0:1] + motion_offsets).astype(np.float32)
    else:
...
def build_irregular_history_offsets(hist_num, query_gap, transition_gaps):
    ...
    for index in range(1, hist_num):
        increment = transition_gaps[(index - 1) % len(transition_gaps)]
        offsets.append(offsets[-1] + increment)
    return offsets


def build_alternating_aux_history_offsets(
    hist_num, sample_index, query_gaps=(2, 4), transition_gaps=(1, 2)
):
    ...
    return build_irregular_history_offsets(hist_num, gap, transition_gaps)
...
def select_causal_temporal_candidates(boundary_ratios, available, *, boundary_band=0.2):
    ...
    """
    ratios = {int(gap): float(value) for gap, value in boundary_ratios.items()}
    availability = {int(gap): bool(value) for gap, value in available.items()}
    ...
    if outside:
        ...
        if remaining:
            selected[1] = min(remaining, key=lambda gap: (abs(ratios[gap] - 1.0), gap))
    else:
    ...
    for role_id in (1, 2):
        ...
        role_satisfied = (
            abs(ratio - 1.0) <= boundary_band if role_id == 1 else ratio > 1.0
        )
        result[role_id] = {
            "gap": gap,
...
def select_uniform_temporal_candidates(boundary_ratios, available, *, seed_parts=()):
    ...
    ordered = sorted(
        valid_gaps,
        key=lambda gap: (
            hashlib.sha256(f"causal-uniform::{prefix}::{gap}".encode("utf-8")).digest(),
            gap,
        ),
    )
    result = {}
