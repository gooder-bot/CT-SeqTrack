...
def main():
    ...
    modes = sum(
        bool(value) for value in (args.rows, args.selection_rows, args.audit_rows)
    )
    if modes != 1:
        parser.error("choose exactly one of --rows, --selection-rows, --audit-rows")
    ...
    def scene_digest(path, supplied, partition):
        ...
        if not isinstance(partition_row, dict) or not partition_row.get(
            "content_sha256"
        ):
    ...
    checkpoint_sha256 = (
        sha256_file(args.checkpoint) if args.checkpoint else args.checkpoint_sha256
    )
    if args.config:
        resolved_config = load_yaml_config(args.config)
        configure_ct_variant(resolved_config)
        config_sha256 = sha256_json(action_calibration_config_identity(resolved_config))
    else:
    ...
    if args.rows:
        ...
        artifact = select_action_thresholds(
            ...
            max_harmful_upper=args.max_harmful_upper,
            **common,
        )
    else:
        ...
        audit_manifest_sha256 = scene_digest(
            args.audit_scene_manifest,
            args.audit_scene_manifest_sha256,
            "calibration_audit",
        )
        with Path(args.selection_artifact).open("r", encoding="utf-8") as handle:
            selection_artifact = json.load(handle)
        artifact = audit_action_thresholds(
            load_rows(args.audit_rows),
            selection_artifact,
            checkpoint_sha256,
            config_sha256,
            selection_manifest_sha256,
