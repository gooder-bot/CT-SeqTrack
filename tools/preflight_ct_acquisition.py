...
from utils.acquisition_metrics import (
...
def main():
    ...
    parser.add_argument("--rows", required=True,
                        help="JSON/JSONL rows from the fixed-CV sampler pass")
    ...
    if not artifact["passed"]:
        failed = sorted(key for key, value in artifact["criteria"].items()
                        if not value)
