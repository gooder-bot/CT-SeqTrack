"""导出 v28 初始 B0/后续矩阵计划；--execute 只允许启动初始 B0。"""
...
SCHEMA = "ct_seqtrack.run_matrix.v28"
...
def safe_output_directory(directory):
    ...
    if destination == protected or protected in destination.parents:
        raise ValueError("v28 matrix artifacts must not use the protected output/ directory")
    return destination
...
def _source_config(arm, stage):
    ...
    return parent / f"28_{arm}{suffix}.yaml"
...
def build_matrix(stage, data_path, output_directory, python="python"):
    ...
    if stage not in ("initial", "mini", "full"):
        raise ValueError("stage must be initial, mini or full")
    output = safe_output_directory(output_directory)
    categories = ("Car",) if stage in ("initial", "mini") else CATEGORIES
    runs, configs = [], {}
    for category in categories:
        for arm in (("b0",) if stage == "initial" else ARMS):
            source = _source_config(arm, stage)
            config = copy.deepcopy(load_yaml_config(source))
            if (not config.get("ct_enable_v28") or config.get("epoch") != 60
                ...
                raise ValueError(f"invalid scratch v28 formal config: {source}")
            name = f"ct28_{stage}_{category.lower()}_{arm}_seed42_60ep_bs16"
            config.update(category_name=category, path=str(data_path), batch_size=16, workers=12, check_val_every_n_epoch=5)
            ...
            for epoch in LATE_EPOCHS:
                ...
                if arm == "full":
                    ...
                    task["calibration_argv"] = [python, "tools/calibrate_ct_actions.py", "--v28",
    ...
    return {
        "schema": SCHEMA, "stage": stage, "initial_formal_arm": "b0", "working_directory": str(ROOT),
...
def execute_training(manifest, output_directory):
    if manifest.get("stage") != "initial" or len(manifest.get("runs", [])) != 1 or manifest["runs"][0]["arm"] != "b0":
        raise ValueError("v28 automatic execution supports only the initial B0 run; matrix commands remain pending")
    output = safe_output_directory(output_directory)
...
def main(argv=None):
    ...
    parser.add_argument("--stage", default="initial", choices=("initial", "mini", "full"))
    ...
    parser.add_argument("--execute", action="store_true", help="explicitly launch initial B0 only; never run the later matrix automatically")
