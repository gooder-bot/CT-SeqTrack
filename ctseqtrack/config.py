...
def configure_ct_variant(config):
    ...
    if variant not in VARIANTS:
        raise ValueError("ct_variant must be b0, b1, full_minus_b3 or full")
    ...
    values = {
        ...
        "ct_point_evidence_contract_version": 2,
    ...
    forbidden = (
        ...
        "use_b3_risk_router",
        "use_joint_proposal_fusion",
        "use_advantage_proposal_fusion",
    )
    enabled = [name for name in forbidden if bool(get_config(config, name, False))]
    if enabled:
        raise ValueError(
    ...
    prior_mode = (
        str(get_config(config, "ct_prior_mode", "learned_physical")).strip().lower()
    )
    if prior_mode != "learned_physical":
        raise ValueError(
            "formal v25 trains the learned physical prior from scratch; "
    ...
    if variant == "b0":
        ...
        # every B0 entry safe when the common profile evolves.
        for name in (
            ...
            set_config(config, name, False)
    memory_mode = str(get_config(config, "ct_memory_mode", "none")).strip().lower()
    if memory_mode not in ("none", "empty", "real", "time_misaligned"):
        raise ValueError("ct_memory_mode must be none, empty, real or time_misaligned")
    set_config(config, "ct_memory_mode", memory_mode)
    time_mode = (
        str(
            get_config(
                config, "ct_time_mode", get_config(config, "dynamics_time_mode", "true")
            )
        )
        .strip()
        .lower()
