...
def tracking_metrics(rows):
    ...
    success_curve = np.asarray(
        [np.mean(overlaps >= threshold) for threshold in np.linspace(0.0, 1.0, 21)]
    )
    precision_curve = np.asarray(
        [np.mean(distances <= threshold) for threshold in np.linspace(0.0, 2.0, 21)]
    )
    return {
        "success": float(np.trapz(success_curve, x=np.linspace(0.0, 1.0, 21)) * 100.0),
        "precision": float(
            np.trapz(precision_curve, x=np.linspace(0.0, 2.0, 21)) * 50.0
        ),
    }
...
def paired_scene_bootstrap(baseline_rows, method_rows, seed=42, resamples=20000):
    ...
    identities_by_scene = {
        scene: [identity for identity in sorted(baseline) if identity[0] == scene]
        for scene in scenes
    }
