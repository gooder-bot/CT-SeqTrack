...
SCORE_DEFINITION = "sigmoid(help_logit)*(1-sigmoid(harm_logit))"
...
B1_CALIBRATION_CONFIG_FIELDS = (
    ...
    "use_b1motion_v3",
    "motion_v3_hidden_dim",
    "motion_v3_step_dim",
    "motion_v3_temporal_backend",
    "motion_v3_cfc_backbone_units",
    "time_scale",
    "motion_v3_time_scale",
    "motion_v3_min_delta_t",
    "motion_v3_max_delta_t",
    "motion_v3_max_speed",
    "motion_v3_max_displacement",
    "motion_v3_min_direction_speed",
    "motion_v3_log_sigma_min",
    "motion_v3_log_sigma_max",
...
def require_selective_calibration(calibrated, inference_mode):
    ...
    if mode in ("full", "full_selective", "selective") and not bool(calibrated):
        raise RuntimeError(
...
def b1_calibration_config_sha256(config):
    ...
    return sha256_json({key: getter(key, None) for key in B1_CALIBRATION_CONFIG_FIELDS})


def validate_b1_calibration_state(calibration, state_dict):
    """Bind B1 calibration metadata to the checkpoint scale buffer."""
    ...
    keys = [
        ...
        if key.endswith("physical_motion_encoder.log_sigma_calibration")
    ]
    if len(keys) != 1 or not isinstance(expected, (list, tuple)):
        raise RuntimeError("B1 calibration metadata/buffer contract is incomplete")
    actual = state_dict[keys[0]].detach()
    expected_tensor = torch.as_tensor(
        expected, dtype=actual.dtype, device=actual.device
    )
    if expected_tensor.shape != actual.shape or not torch.equal(
        expected_tensor, actual
    ):
        raise RuntimeError("B1 calibration metadata does not match checkpoint buffer")
    return True
...
def action_calibration_config_identity(config):
    ...
    fields = (
        ...
        "ct_memory_mode",
        "ct_partition_scheme",
        "ct_b2_target_bb_scale",
        "ct_prior_mode",
        "ct_time_mode",
        "ct_search_presence_threshold",
        "ct_router_radius_base",
        "ct_router_radius_per_second",
        "ct_router_radius_max",
        "ct_router_hidden_dim",
        "ct_router_help_margin",
        "ct_router_h3_margin",
    )
...
def _finite_scene_rows(rows, partition, *, require_selective=False, thresholds=None):
    normalized = _finite_rows(rows)
    for item in normalized:
        if "partition_group_key" not in item:
            raise ValueError("v25 calibration rows require partition_group_key (scene)")
        item["partition_group_key"] = str(item["partition_group_key"])
        if str(item.get("partition", "")) != str(partition):
            raise ValueError(f"v25 calibration row is not in {partition}")
        if require_selective:
            mode = (
                str(item.get("rollout_mode", item.get("proposal_inference_mode", "")))
                .strip()
                .lower()
            )
            if mode != "selective":
            ...
            if thresholds is not None:
                for field, expected in (
                    ...
                    if field not in item or not np.isclose(
                        float(item[field]), float(expected), rtol=0.0, atol=1e-12
                    ):
                        raise ValueError(
...
def _scene_metrics(selected, total_rows):
    ...
    metrics["scenes"] = int(len({row["partition_group_key"] for row in selected}))
    return metrics
...
def scene_bootstrap_bounds(selected, seed=42, resamples=2000):
    ...
    for index in range(int(resamples)):
        picked = rng.choice(keys, size=len(keys), replace=True)
        sampled_rows = [row for key in picked for row in groups[str(key)]]
...
def select_action_thresholds(
    ...
    presence_values = np.unique(
        np.quantile([row["presence_score"] for row in rows], np.linspace(0.0, 0.95, 20))
    )
    action_values = np.unique(
        np.quantile([row["action_score"] for row in rows], np.linspace(0.0, 0.99, 50))
    )
    feasible = []
    evaluated = 0
    for presence_threshold in presence_values:
        for action_threshold in action_values:
            ...
            if (
                metrics["actions"] < int(min_actions)
                or metrics["scenes"] < int(min_scenes)
                or metrics["coverage"] < float(min_coverage)
            ):
                continue
            evaluated += 1
            bounds = scene_bootstrap_bounds(selected, seed=seed, resamples=resamples)
            candidate = {
            ...
            if (
                bounds["harmful_rate_upper_95"] <= max_harmful_upper
    ...
    if feasible:
        chosen = max(
            feasible,
            key=lambda item: (item["center_gain"] + item["iou_gain"], item["coverage"]),
        )
    else:
        chosen = {
    ...
    artifact = {
        ...
        "selection_scene_manifest_sha256": str(selection_scene_manifest_sha256),
        "score_definition": SCORE_DEFINITION,
        "selection_partition": "calibration_select",
        "selection_scene_keys": sorted({row["partition_group_key"] for row in rows}),
        "thresholds": {
            "presence": chosen["presence_threshold"],
            "action": chosen["action_threshold"],
        },
        "selection_diagnostics": {
            key: value for key, value in chosen.items() if not key.endswith("threshold")
        },
        "requirements": {
...
def validate_threshold_selection(
    artifact, checkpoint_sha256, config_sha256, selection_scene_manifest_sha256
):
    if not isinstance(artifact, dict) or artifact.get("schema") != SELECTION_SCHEMA:
    ...
    if (
        artifact.get("selection_partition") != "calibration_select"
    ...
    if (
        not isinstance(scene_keys, list)
        or not scene_keys
...
def audit_action_thresholds(
    ...
    if str(selection_scene_manifest_sha256) == str(audit_scene_manifest_sha256):
        raise ValueError(
    ...
    rows = _finite_scene_rows(
        rows, "calibration_audit", require_selective=True, thresholds=thresholds
    )
    audit_scene_keys = sorted({row["partition_group_key"] for row in rows})
    ...
    selected = _selected(
        rows, float(thresholds["presence"]), float(thresholds["action"])
    )
    metrics = _scene_metrics(selected, len(rows))
    bounds = scene_bootstrap_bounds(selected, seed=seed, resamples=resamples)
    criteria = {
        ...
        "audit_iou_gain_lower_95_nonnegative": (bounds["iou_gain_lower_95"] >= 0.0),
    }
    artifact = {
        "schema": AUDIT_SCHEMA,
        ...
        "threshold_selection_artifact_sha256": selection_artifact["artifact_sha256"],
        "score_definition": SCORE_DEFINITION,
        "selection_partition": "calibration_select",
...
def calibrate_actions(
    ...
    presence_values = np.quantile(
        [row["presence_score"] for row in rows], np.linspace(0.0, 0.95, 20)
    )
    action_values = np.quantile(
        [row["action_score"] for row in rows], np.linspace(0.0, 0.99, 50)
    )
    feasible = []
    evaluated = []
    for presence_threshold in np.unique(presence_values):
        for action_threshold in np.unique(action_values):
            ...
            if (
                ...
                continue
            bounds = tracklet_bootstrap_bounds(selected, seed=seed, resamples=resamples)
            candidate = {
                "presence_threshold": float(presence_threshold),
                "action_threshold": float(action_threshold),
                **metrics,
...
def risk_coverage_curve(
    rows, presence_threshold=0.0, seed=42, resamples=2000, points=50
):
    ...
    thresholds = np.unique(
        np.quantile(
            ...
            np.linspace(0.0, 1.0, int(points)),
        )
    )[::-1]
    curve = []
    for threshold in thresholds:
        selected = [row for row in eligible if row["action_score"] >= float(threshold)]
        curve.append(
            {
                "action_threshold": float(threshold),
                **_metrics(selected, len(rows)),
                **tracklet_bootstrap_bounds(selected, seed=seed, resamples=resamples),
            }
        )
    return curve
...
def validate_action_calibration(
    ...
    config_sha256,
    tracklet_manifest_sha256,
    selection_scene_manifest_sha256=None,
):
    ...
    if artifact.get("schema") == AUDIT_SCHEMA:
        if artifact.get("selection_scene_manifest_sha256") == artifact.get(
            "audit_scene_manifest_sha256"
        ):
            raise ValueError("selection and audit scene manifests must be disjoint")
        if (
            artifact.get("selection_partition") != "calibration_select"
            or artifact.get("audit_partition") != "calibration_audit"
            or artifact.get("safety_population") != "calibration_audit_only"
        ):
            raise ValueError("v25 calibration must separate selection and audit scenes")
        selection_scenes = {
            str(key) for key in artifact.get("selection_scene_keys", [])
        }
        audit_scenes = {str(key) for key in artifact.get("audit_scene_keys", [])}
        if (
            not selection_scenes
            or not audit_scenes
            or selection_scenes.intersection(audit_scenes)
        ):
            raise ValueError("v25 calibration scene populations are empty or overlap")
