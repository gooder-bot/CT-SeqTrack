...
def acquisition_config_identity(config):
    ...
    keys = {
        ...
        "ct_recursive_candidate_views",
        "ct_recursive_tracklet_slots",
        "ct_recursive_rollout_horizons",
        "ct_recursive_reseed_enabled",
        "ct_partition_seed",
        "ct_protocol_version",
        "ct_training_reanchor_policy",
        "ct_b0_rng_protocol",
        "ct_b0_rng_shift_control",
        "ct_partition_scheme",
        "ct_candidate_sampling_seed_scope",
        "ct_point_evidence_contract_version",
        "ct_b2_target_bb_scale",
        "ct_router_partition",
        "ct_auxiliary_microbatch_size",
        "ct_recovery_candidate_policy",
        "ct_candidate_policy",
        "ct_temporal_candidate_gaps",
        "ct_temporal_boundary_band",
        "ct_presence_training_scope",
        "ct_endpoint_quota",
        "ct_tube_quota",
        "ct_expansion_point_count",
        "ct_search_training_history",
        "ct_search_min_points",
        "ct_tube_max_length",
        "ct_tube_max_width",
        "ct_motion_max_speed",
        "ct_motion_max_acceleration",
        "ct_motion_max_displacement",
        "ct_motion_acceleration_weight",
        "ct_search_extension_voxel_size",
        "ct_search_min_total_points",
        "ct_search_min_extension_points",
        "ct_search_min_extension_voxels",
        "ct_search_endpoint_ratio",
        "ct_search_sparse_base_points",
        "search_v3_use_dynamic_sigma",
        "search_v3_fixed_margin_parallel",
        "search_v3_fixed_margin_perpendicular",
        "search_v3_coverage_scale",
        "search_v3_standardized_residual_q90_parallel_perpendicular",
        "trajectory_search_base_length",
        "trajectory_search_base_width",
        "trajectory_search_sigma_parallel_scale",
        "trajectory_search_sigma_perpendicular_scale",
        "trajectory_search_min_displacement",
        "trajectory_search_min_delta_t",
        "trajectory_search_min_gap_ratio",
        "motion_v3_min_direction_speed",
    }
    selected = {str(key): source[key] for key in sorted(keys) if key in source}
...
def summarize_acquisition_rows(rows):
    ...
    for source in rows:
        ...
        sampled_valid = _number(
            row, "sampled_count", _number(row, "extension_pool_count")
        )
        if sampled_targets > pool_targets:
            raise ValueError("sampled_target_count cannot exceed pool_target_count")
        ...
        if has_explicit_sampled_count and sampled_targets > sampled_valid:
            raise ValueError("sampled_target_count cannot exceed sampled_count")
        eligible = pool_targets > 0.0
        retained = eligible and sampled_targets > 0.0
        stats["rows"] += 1
        stats["available_rows"] += int(
            bool(row.get("available", _number(row, "extension_pool_count") > 0.0))
        )
        ...
        stats["recovery_positive_rows"] += int(
            bool(row.get("recovery_positive", False))
        )
        stats["recovery_fallback_rows"] += int(
            bool(row.get("recovery_fallback", False))
        )
        role_satisfied = bool(row.get("role_satisfied", candidate_id == 0))
        boundary_ratio = _number(row, "boundary_ratio", 0.0)
        candidate_gap = int(
            row.get("candidate_gap_frames", 1 if candidate_id == 0 else 0)
        )
        ...
        stats["support_truncated_rows"] += int(
            bool(row.get("support_truncated", False))
        )
        ...
        if candidate_id == 0:
            for gap, ratio in dict(row.get("candidate_gap_pool_ratios", {})).items():
                numeric = float(ratio)
                if not math.isfinite(numeric) or numeric < 0.0:
                    raise ValueError(
                        "candidate gap-pool ratios must be finite and non-negative"
                    )
                stats["selection_pool_ratios"].setdefault(str(int(gap)), []).append(
                    numeric
                )
    output = []
    for key in sorted(groups):
        ...
        stats["row_recall"] = (
            stats["retained_eligible_rows"] / eligible if eligible else None
        )
        stats["point_recall"] = (
            stats["sampled_target_count"] / pool_targets if pool_targets else None
        )
        stats["boundary_ratio_mean"] = (
            stats["boundary_ratio_sum"] / stats["boundary_ratio_count"]
            if stats["boundary_ratio_count"]
            else None
        )
        stats["support_truncation_rate"] = (
            stats["support_truncated_rows"] / stats["available_rows"]
            if stats["available_rows"]
            else None
        )
        stats["processing_time_ms_mean"] = (
            stats["processing_time_ms_sum"] / stats["processing_time_ms_count"]
            if stats["processing_time_ms_count"]
            else None
        )
        stats["processing_time_ms_p95"] = _quantile(stats["processing_times_ms"], 0.95)
...
def build_preflight_artifact(
    ...
    seed,
    primary_partition="dev",
    min_target_bearing_rows=100,
    ...
    protocol_version = int(acquisition_identity.get("ct_protocol_version", 24))
    ...
    train_positive = sum(
        ...
        * (role_weights[item["candidate_id"]] if protocol_version >= 25 else 1.0)
        for item in groups
    ...
    candidate_policy = str(
        acquisition_identity.get("ct_candidate_policy", "causal_b1_boundary")
    )
    criteria = {
        ...
        "dev_candidate0_target_bearing_rows_at_least_minimum": bool(
            primary and primary["eligible_rows"] >= int(min_target_bearing_rows)
        ),
        "dev_candidate0_row_retention_at_least_minimum": bool(
            primary
            and primary["row_recall"] is not None
        ...
        criteria.update(
            {
                "dev_candidate1_uniform_available_rows_at_least_minimum": bool(
                    boundary
                    and boundary["available_rows"] >= int(min_target_bearing_rows)
                ),
                "dev_candidate2_uniform_available_rows_at_least_minimum": bool(
                    outside
                    and outside["available_rows"] >= int(min_target_bearing_rows)
                ),
            }
        )
    else:
        criteria.update(
            {
                "dev_candidate1_boundary_role_rows_at_least_minimum": bool(
                    boundary
                    and boundary["role_satisfied_rows"] >= int(min_target_bearing_rows)
                ),
                "dev_candidate2_outside_role_rows_at_least_minimum": bool(
                    outside
                    and outside["role_satisfied_rows"] >= int(min_target_bearing_rows)
                ),
            }
        )
    ...
    targetness_class_weights = {
        "positive": (
            class_total / (2.0 * train_positive) if train_positive > 0 else 0.0
        ),
        "negative": (
            class_total / (2.0 * train_negative) if train_negative > 0 else 0.0
        ),
    ...
    if protocol_version >= 25:
        ...
        statistics["role_weighted_class_counts"] = payload["role_weighted_class_counts"]
...
def validate_preflight_artifact(artifact, config):
    ...
    statistics = {
        ...
        "targetness_class_weights": artifact.get("targetness_class_weights"),
    }
    if protocol_version >= 25:
        ...
        if artifact.get("role_weights") != {"0": 0.5, "1": 0.3, "2": 0.2}:
    ...
    if int(artifact.get("seed", -1)) != int(_config_get(config, "seed", 42) or 42):
        raise ValueError("acquisition preflight seed mismatch")
    if artifact.get("primary_population") != {"partition": "dev", "candidate_id": 0}:
        raise ValueError(
            "acquisition preflight primary population must be dev candidate0"
        )
    requirements = artifact.get("requirements", {})
    if (
        int(requirements.get("min_target_bearing_rows", -1)) < 100
    ...
    if (
        ...
        raise ValueError(
            "acquisition preflight lacks a complete checkpoint-free data " "manifest"
        )
    ...
    if protocol_version >= 25:
        ...
        if (
            not isinstance(scene_manifest, dict)
            or scene_manifest.get("schema") != "ct_seqtrack.scene_partition_manifest.v1"
    ...
    runtime_policy = str(
        _config_get(config, "ct_candidate_policy", "causal_b1_boundary")
    )
    expected_selection = (
        "uniform valid temporal gaps; current GT hidden"
        if runtime_policy == "causal_temporal_uniform"
        else "fixed-cv endpoint boundary/outside; current GT hidden"
    )
    ...
    if runtime_path is None or manifest_path is None:
        ...
        Path(str(runtime_path)).expanduser().resolve()
    ...
    if not isinstance(partitions, list):
        ...
        for partition in ("train", "dev"):
            ...
            if (
                ...
                or (protocol_version >= 25 and int(item.get("dropped_rows", -1)) != 0)
