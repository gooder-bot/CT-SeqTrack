...
_RESUME_TRANSPORT_FIELDS = {
    "acquisition_preflight",
    "b1_calibration_artifact_path",
    "b2_method_promotion",
    "cfg",
    "checkpoint",
    "ct_source_checkpoint_epoch",
    "log_dir",
}
...
def online_candidate_state_consistent(processed, target_size):
    ...
    if search_history is not None and motion_history is None:
        raise RuntimeError("B2 Search history exists without its B1 motion history")
    history_consistent = bool(
        search_history is None or np.array_equal(motion_history, search_history)
    )
...
def resolved_training_config_sha256(config):
    """Hash the full resolved training configuration, excluding I/O transport.

    Artifact payloads injected into the config remain covered. Only CLI paths
    that locate the already-bound checkpoint/artifacts and the output folder
    are excluded, so a same-run resume may move files without becoming a
    cross-configuration finetune.
    """
    source = config if isinstance(config, dict) else vars(config)
    semantic = {
        str(key): value
        for key, value in source.items()
        if str(key) not in _RESUME_TRANSPORT_FIELDS
    }
    encoded = json.dumps(
        semantic,
        sort_keys=True,
...
def build_online_resume_contract(config):
    ...
    fields = {
        "resolved_training_config_sha256": resolved_training_config_sha256(config),
        ...
        "time_mode": str(
            _get(config, "ct_time_mode", _get(config, "dynamics_time_mode", "true"))
        ),
        "fixed_delta_t": float(_get(config, "dynamics_fixed_delta_t", 0.5)),
        "time_manifest": _get(config, "dynamics_time_manifest"),
        "training_state_policy": str(
            _get(config, "ct_training_state_policy", "legacy")
        ),
        "module_isolation": str(_get(config, "ct_module_isolation", "legacy")),
        "seed": int(_get(config, "seed", 42) or 42),
        "joint_contract_version": int(_get(config, "ct_joint_contract_version", 1)),
        "protocol_version": int(_get(config, "ct_protocol_version", 24)),
        "online_recursive_training": bool(
            _get(config, "ct_online_recursive_training", False)
        ),
        "use_ct_joint_full": bool(_get(config, "use_ct_joint_full", False)),
        ...
        "rollout_horizons": tuple(
            int(value) for value in _get(config, "ct_recursive_rollout_horizons", [1])
        ),
        "reseed_enabled": bool(_get(config, "ct_recursive_reseed_enabled", False)),
        "b0_rng_shift_control": bool(_get(config, "ct_b0_rng_shift_control", False)),
        "training_reanchor_policy": str(
            _get(config, "ct_training_reanchor_policy", "legacy_boolean")
        ),
        "b0_rng_protocol": str(_get(config, "ct_b0_rng_protocol", "legacy_boolean")),
        "partition": str(_get(config, "ct_router_partition", "train")),
        "partition_seed": int(_get(config, "ct_partition_seed", 42)),
        "partition_scheme": str(_get(config, "ct_partition_scheme", "tracklet_v1")),
        "optimizer": str(_get(config, "optimizer", "Adam")).lower(),
        "base_lr": base_lr,
        "b0_lr": float(_get(config, "ct_b0_lr", base_lr)),
        "b1_lr": float(_get(config, "ct_b1_lr", _get(config, "ct_plugin_lr", base_lr))),
        "b2_lr": float(_get(config, "ct_b2_lr", _get(config, "ct_plugin_lr", base_lr))),
        "b3_lr": float(_get(config, "ct_b3_lr", _get(config, "ct_plugin_lr", base_lr))),
        ...
        "separate_optimizers": bool(_get(config, "ct_separate_optimizers", False)),
        "manual_amp_enabled": bool(_get(config, "ct_manual_amp_enabled", False)),
        ...
        "check_val_every_n_epoch": int(_get(config, "check_val_every_n_epoch", 1)),
        "canonical_batch_size": int(_get(config, "batch_size", 1)),
        "auxiliary_microbatch_size": int(
            _get(config, "ct_auxiliary_microbatch_size", 16)
        ),
        "memory_mode": str(_get(config, "ct_memory_mode", "real")),
        "base_evidence_mode": str(_get(config, "ct_base_evidence_mode", "full")),
        "recovery_candidate_policy": str(
            _get(config, "ct_recovery_candidate_policy", "off")
        ),
        "candidate_policy": str(_get(config, "ct_candidate_policy", "legacy_spatial")),
        "temporal_candidate_gaps": tuple(
            int(value)
            for value in _get(config, "ct_temporal_candidate_gaps", [2, 4, 8])
        ),
        "temporal_boundary_band": float(_get(config, "ct_temporal_boundary_band", 0.2)),
        "candidate_sampling_seed_scope": str(
            _get(config, "ct_candidate_sampling_seed_scope", "candidate_role")
        ),
        "point_evidence_contract_version": int(
            _get(config, "ct_point_evidence_contract_version", 1)
        ),
        "b2_target_bb_scale": float(
            _get(config, "ct_b2_target_bb_scale", _get(config, "bb_scale", 1.25))
        ),
        "presence_training_scope": str(
            _get(config, "ct_presence_training_scope", "all_candidates")
        ),
        "candidate_loss_weights": (0.5, 0.3, 0.2),
        "initialization_policy": str(
            _get(
                config,
                "ct_initialization_policy",
                _get(config, "ct_b0_initialization_policy", "legacy"),
            )
        ),
        "targetness_class_weight_source": str(
            _get(config, "ct_targetness_class_weight_source", "legacy")
        ),
        "targetness_positive_weight": float(
            _get(config, "ct_targetness_positive_weight", 1.0)
        ),
        "targetness_negative_weight": float(
            _get(config, "ct_targetness_negative_weight", 1.0)
        ),
        "acquisition_preflight_statistics_sha256": (
            preflight.get("statistics_sha256") if isinstance(preflight, dict) else None
        ),
...
def validate_online_resume_contract(checkpoint, config):
    ...
    if observed.get("schema") != ONLINE_RESUME_SCHEMA:
        raise ValueError(
            "online resume checkpoint does not carry " f"{ONLINE_RESUME_SCHEMA}"
        )
    ...
    if mismatches:
        raise ValueError(
            "online resume contract mismatch: " + ", ".join(sorted(mismatches))
        )
    ...
    if (
        ...
        or recursive_boundary.get("schema") != "ct_seqtrack.recursive_state_boundary.v1"
...
def validate_scratch_training_contract(config):
    ...
    if not bool(_get(config, "ct_online_recursive_training", False)):
        raise ValueError("scratch-only v25 requires online recursive training")
    ...
    if str(_get(config, "net_model", "seqtrack3d")) == "ctseqtrack":
        ...
        if str(_get(config, "ct_time_mode", "true")) == "shuffled" and not _get(
            config, "dynamics_time_manifest"
        ):
            errors.append("ct_time_mode=shuffled requires dynamics_time_manifest")
    ...
    if base_lr != 1e-4 or b0_lr != 1e-4:
        errors.append("matched scratch B0 requires lr=ct_b0_lr=1e-4")
    ...
    if protocol_version >= 25:
        require_equal("ct_training_reanchor_policy", "periodic_past_gt", None)
        require_equal("ct_recursive_reseed_enabled", True, False)
        require_equal("ct_b0_rng_protocol", "post_observation_shift_v1", None)
        require_equal("ct_b0_rng_shift_control", True, False)
        require_equal("ct_partition_scheme", "scene_v2", None)
        require_equal("ct_candidate_sampling_seed_scope", "physical_frame", None)
    ...
    if b3 and not (b2 and b1):
        errors.append("B3 scratch Full requires learned B1 and B2 from epoch 0")
    if b1 or b2 or b3:
        ...
        for module_name in ("b1", "b2", "b3"):
            if bool(_get(config, f"ct_enable_{module_name}", False)):
                module_lr = float(_get(config, f"ct_{module_name}_lr", plugin_lr))
                if module_lr != 1e-4:
                    errors.append(
    ...
    if b2:
        ...
        if protocol_version < 25:
            ...
            if b2_scale not in (1.0, 1.25):
                errors.append("v25 ct_b2_target_bb_scale must be 1.0 or 1.25")
    ...
    if errors:
        raise ValueError("invalid scratch training contract: " + "; ".join(errors))
...
def build_b2_method_contract(config):
    ...
    keys = (
        ...
        "rollout_horizons",
        "reseed_enabled",
        "training_reanchor_policy",
        "b0_rng_shift_control",
        "b0_rng_protocol",
        "partition",
        "partition_seed",
        "partition_scheme",
        ...
        "lr_decay_rate",
        "b0_gradient_clip",
        "plugin_gradient_clip",
        "canonical_batch_size",
        "auxiliary_microbatch_size",
        "initialization_policy",
        "recovery_candidate_policy",
        "candidate_policy",
        "candidate_sampling_seed_scope",
        "point_evidence_contract_version",
        "b2_target_bb_scale",
        "temporal_candidate_gaps",
        "temporal_boundary_band",
        "presence_training_scope",
        "candidate_loss_weights",
        "acquisition_preflight_statistics_sha256",
        "targetness_class_weight_source",
        "targetness_positive_weight",
...
def validate_b2_method_promotion(promotion, config):
    if (
        ...
        or promotion.get("schema") != "ct_seqtrack.b2_evidence_promotion.v4"
    ...
    if mismatches:
        raise ValueError(
            "B2 method promotion/config mismatch: " + ", ".join(sorted(mismatches))
        )
