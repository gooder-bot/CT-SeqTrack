...
class FullGatedCfCCell(nn.Module):
    """One explicit-time, full-gated closed-form continuous update."""
    ...
    def __init__(self, input_size, hidden_size, backbone_units=105):
        ...
        self.first_state = nn.Linear(
            self.backbone_units, self.hidden_size)
        self.second_state = nn.Linear(
            self.backbone_units, self.hidden_size)
    ...
    def _elapsed_column(self, elapsed_time, batch_size, reference):
        ...
        if elapsed_time.numel() == 1:
            elapsed_time = elapsed_time.reshape(1, 1).expand(
                batch_size, 1)
        elif elapsed_time.numel() == batch_size:
            elapsed_time = elapsed_time.reshape(batch_size, 1)
        else:
            raise ValueError(
                "CfC elapsed_time must be scalar or one value per batch")
    ...
    def forward(self, input_value, hidden_state, elapsed_time):
        if (input_value.dim() != 2
                or input_value.shape[1] != self.input_size):
            raise ValueError(
                f"CfC input must have shape [B,{self.input_size}]")
        if hidden_state.shape != (
                input_value.shape[0], self.hidden_size):
            raise ValueError(
        ...
        gate = torch.sigmoid(
            self.time_a(shared) * elapsed + self.time_b(shared))
