...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz, features, npoint, return_idx=False):
        ...
        for i in range(len(self.groupers)):
            ...
            )  # (B, C, npoint, nsample)  #torch.Size([8, 14, 512, 32]) 多了一个维度

            new_features = self.mlps[i](new_features)  # (B, mlp[-1], npoint, nsample) #torch.Size([8, 128, 512, 32])升通道数
            new_features = F.max_pool2d(
                new_features, kernel_size=[1, new_features.size(3)]
            )  # (B, mlp[-1], npoint, 1) torch.Size([8, 128, 512, 1]) 32被pool掉了
            new_features = new_features.squeeze(-1)  # (B, mlp[-1], npoint) torch.Size([8, 128, 512]) 恢复原型
