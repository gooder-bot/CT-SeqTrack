...
def build_scene_manifest(scene_splits, version, seed=42):
    ...
    ranked = sorted(set(scene_splits[source]), key=lambda name: hashlib.sha256(
        f'ct27|{int(seed)}|{name}'.encode('utf-8')).hexdigest())
    expected = 8 if mini else 350
    if len(ranked) != expected:
        raise ValueError(f'v27 {source} requires {expected} scenes, got {len(ranked)}')
    test = sorted(set(scene_splits[evaluation]))
    if len(test) != (2 if mini else 150) or set(test) & set(ranked):
        raise ValueError('v27 official evaluation split count/overlap mismatch')
    roles = {'train': sorted(set(scene_splits[source])),
             'calibration': ranked[6:7] if mini else ranked[315:332],
             'dev': ranked[7:] if mini else ranked[332:], 'test': test}
    result = dict(schema='ct_seqtrack.scene_protocol.v28', version=str(version),
                  split_seed=int(seed), training_source=source, evaluation_source=evaluation,
                  parameter_training_overlap=True, scenes=roles)
