...
def crop_pc_axis_aligned_with_aroundboxs(PC, box, around_boxs, offset=0, scale=1.0, return_mask=False):
    ...
    around_boxs_tmp = copy.deepcopy(around_boxs)

    box_tmp.wlh = box_tmp.wlh * scale
    for box in around_boxs_tmp:
        box.wlh = box.wlh* scale 

    all_boxs =  [box_tmp] + around_boxs_tmp

    max_coords = np.array([np.max(box.corners(), axis=1) + offset for box in all_boxs])
    min_coords = np.array([np.min(box.corners(), axis=1) - offset for box in all_boxs])

    maxi = np.max(max_coords, axis=0)
    mini = np.min(min_coords, axis=0)
