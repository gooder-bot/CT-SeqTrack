...
这份文件用于每次开始工作前快速整理研究思路。工程执行清单见 `need_to_do.md`，验收日志和命令见 `log.md`。
...
### 贡献 1：Timestamp-native Seq2Seq 3D SOT

把 SeqTrack3D 的固定伪时间替换为真实 timestamp：

- 点特征时间通道使用 `t_i - t_current`。
- box corner token 使用同一套 `delta_T`。
- 训练和测试都提供一致的 `timestamps / delta_t / delta_T / current_delta_t`。
- 时间编码支持 `raw / mlp / fourier`，但保持 scalar-preserving，避免把涨点归因到模型容量。
- 历史框差分按真实 `delta_t` 计算速度和角速度。

这是最小可验证贡献，也是后续 P3-P5 的地基。
...
当前仓库可以作为 P0-P3 的工程快照提交：真实时间字段、scalar-preserving `TimeEncoding`、Dynamics / Velocity Branch 和低占用训练检查脚本都已落地。默认配置保持保守，P3 分支默认关闭，便于后续把性能变化归因到明确的实验开关。

需要注意：这一步只完成工程验收，尚未形成正式论文结果。后续主线应先做 P4，再统一跑 `fixed pseudo time / raw real time / time encoding / dynamics / TWC` 的消融，避免边写边改导致结论混在一起。
...
当前代码已实现 P3，默认关闭。服务器 `check_forward_batch.py` + loss smoke test 以及 `check_train_steps.py --max-steps 2` 均已通过，P3 工程验收完成，下一步进入 P4。
